...
export const ja_JP: TranslationMap = {
  ...
  newSession: {
    ...
    incognito: "シークレット",
    incognitoDescription: "Gateway が再起動するまでの間だけこのスレッドを保持します",
  ...
  login: {
    ...
    deviceAuthMigration: {
      banner: "このブラウザーはアップデート後に一度だけデバイス承認が必要です。",
      action: "このブラウザーを保護",
      secureContextRequired:
        "このレガシーブラウザーは一時的に利用できます。HTTPS または localhost で開き直し、デバイス ID で保護してください。",
      pendingUnavailable:
        "ブラウザーのペアリングリクエストはまだ利用できません。しばらくしてから再試行してください。",
  ...
  chat: {
    ...
    composer: {
      ...
      offlineQueuedHint:
        "オフライン — {count}件がキュー待ち。接続が復帰するとメッセージが送信されます。",
